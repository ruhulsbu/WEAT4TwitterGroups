...
conservative_tweet_list = [u'tcot', u'tlot', u'ocra', u'gop', u'republicans', u'teaparty', u'right']
conservative_tweet_dic = {}
for i in range(0, len(conservative_tweet_list)):
	conservative_tweet_dic[conservative_tweet_list[i]] = True
...
def main(link, file_write):
	...
	for line in tweets_file:
		...
		if tweet.has_key(u'delete') == True \
			or not "en" in tweet["lang"]:
			#or not "US" in tweet["place"]["country_code"]:
			continue
		...
		if len(entities["hashtags"]) > 0:
			...
			for entry in entities["hashtags"]:
				...
				if conservative_tweet_dic.has_key(entry[u'text']):
					conservative_tweet = True
					break
		"""
		for term in tokenized_tweet:
			if term.strip().startswith("#"):
				if conservative_tweet_dic.has_key(term.strip()[1:]):
					conservative_tweet = True
					break	
		
		if not conservative_tweet:
			continue
...
def input_function(start, end, process_no):
	...
	file_write = gzip.open("./dataset_conservative/tweets_" + str(process_no + 1) + ".gz", "wt")
